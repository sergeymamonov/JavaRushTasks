...
public class Solution {
    ...
    public void recurse(final String expression, int countOperation) {
        ...
        // блок 2. если строка - не число
            ...
            String trig = "trig";
            if (n > 2) {
                trig = expr.substring(n - 3, n);
            }        //выделили возможную тригонометрию
    ...
    public String mathStr(String temp) {
        ...
        if (temp.contains("^")) {
            ...
            if (!temp.substring(1, temp.indexOf("+")).contains("-")) {
                sign = "+";
            } else {
                sign = "-";
            }
        ...
        switch (sign) {
            ...
            case "-":
                if (temp.indexOf(sign) != 0) {
                    ...
                    num = operandNum(temp, temp.indexOf(sign, 1));
                }
                result = String.valueOf(roundDouble(num[0] - num[1], 4));
                break;
        }
        ...
        fin = (int) num[3];
        if (st != 0) {
            result = temp.substring(0, st).concat(result);
        }
        if (fin != (temp.length() - 1)) {
            result = result.concat(temp.substring(fin + 1));
        }
        return result;
    }
    ...
    public int count(String expr) {
        ...
        for (int i = 0; i < signt.length; i++) {
            if (expr.contains(signt[i])) {
                String[] f = expr.split(signt[i]);
                result += (f.length - 1);
            }
        }
        char[] sign = {'^', '*', '/', '+', '-'};
